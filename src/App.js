...
function App() {
  // const yourSmartContract = "0xfc1c3dea6fbb5b27aabb727557b50844841e453c";
    ...
    const loadWeb3 = async () => {
      try {
        if (window.ethereum) {
          ...
          // const contract = new window.web3.eth.Contract(XU_Token.abi, yourSmartContract);
          // setXUTokenContract(contract);

          // const xuTokenBalance = await contract.methods.balanceOf(accounts[0]).call();
  ...
  const handleTransfer = async () => {
    try {
      ...
      //   // const xuTokenBalanceBefore = await xuTokenContract.methods.balanceOf(account).call();
      ...
      //   const result = await xuTokenContract.methods.transfer(recipient, weiAmount).send({
      //     from: account,
      ...
      //   const xuTokenBalanceAfter = await xuTokenContract.methods.balanceOf(account).call();
    ...
    <div className="container mt-5 wrapper">
      ...
      {connected && (
        <form className="mb-3">
          ...
          <button type="button" className="btn btn-primary mt-4" onClick={handleTransfer}>
            Chuyển Token
          </button>
        </form>
      )}
